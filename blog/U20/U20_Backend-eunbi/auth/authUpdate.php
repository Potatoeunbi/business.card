<?php
    ...
    if(mysqli_fetch_array($key)){
            ...
            if($form_data!=''){
                $sql = "UPDATE list_admin SET admin_level='".implode(',', $form_data)."' WHERE admin_account='".$id."';";
                $row = $db->query($sql);
                echo "<script>alert('권한 수정되었습니다.'); location.href='../account_user.php';</script>";
                exit;
            }else{
                $sql = "UPDATE list_admin SET admin_level='authEntrysRead,authEntrysUpdate,authEntrysDelete,authEntrysCreate' WHERE admin_account='".$id."';";
                $row = $db->query($sql);
                echo "<script>alert('계정 생성되었습니다.'); location.href='../account_user.php';</script>";
                exit;
            }
