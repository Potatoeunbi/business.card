<?php
    ...
    if(!mysqli_fetch_array($akey)&&!mysqli_fetch_array($jkey)){
            ...
            if($form_data!=''){
                $sql = " INSERT into list_admin (admin_account, admin_password, admin_name, admin_level, admin_datetime)  values ('".$id."','".$pw_hash."','".$name."','".implode(",", $form_data)."','".$create_time."');";
                $row = $db->query($sql);
                echo "<script>alert('계정 생성되었습니다.'); location.href='../signup.php';</script>";
                exit;
            }
            //아래 부분은 생각을 해봐야 될 듯하다. 기본 값으로 모든 권한을 줘야 할 지, 아무 권한도 주지 말아야 할 지는 잘 모르겠다.
            else{
                $sql = " INSERT into list_admin (admin_account, admin_password, admin_name, admin_datetime)  values ('".$id."','".$pw_hash."','".$name."','".$create_time."');";
                $row = $db->query($sql);
                echo "<script>alert('계정 생성되었습니다.'); location.href='../signup.php';</script>";
                exit;
            }
